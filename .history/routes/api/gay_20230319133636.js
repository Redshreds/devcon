class Node {
    constructor(row, col, value) { this.id = row.toString() + + col.toString();
    this.row = row;
    this.col = col;
    this.value = value;
    ...
    // w is the width of the graph and h is the height function aStarAlgorithm(startRow, startCol, endRow, endCol, graph) { const nodes = initializeNodes(graph);
    ...
    const currentMinDistanceNode = nodes ToVisit.remove();
    ...
    nodes ToVisit.insert(neighbor);
    } else {
    nodes ToVisit.update(neighbor);
    }
    }
    }
    return reconstructPath(endNode); }
    _function initializeNodes(graph) { = [];
    const nodes
    for (const [i, row] of graph.entries()) { nodes.push([]); 
      for (const [j, value] of row.entries()) { const node = new Node(i, j, value); 
      nodes[i].push(node);
    }
    }
    return nodes;
    ...
    const currentCol currentNode.col;
      const endRow = endNode.row; const endCol = endNode.col;
    return Math.abs(currentRow - endRow) + Math.abs(cur }
